...
def analyze_entries(streaks, stop_position):
    """
    REVERSED POLARITY (Run C).
    TX label now uses direction-change criterion (was MK).
    MK label now uses streak-depth criterion per T (was TX).

    Both returned dicts are keyed by threshold. TX (direction-change)
    has the same content for every T; MK (streak-depth) varies by T.
    """
    tx_entries = {t: [] for t in THRESHOLDS}
    mk_entries = {t: [] for t in THRESHOLDS}

    direction_changes = []
    streak_depths = {t: [] for t in THRESHOLDS}

    for start_pos, side, streak_len in streaks:
        # Direction-change criterion (now TX)
        if start_pos > 0 and start_pos + 1 <= stop_position:
            busted = streak_len >= BUST_STEPS + 1
            direction_changes.append(busted)
        # Streak-depth criterion per T (now MK)
        for t in THRESHOLDS:
            if streak_len >= t and start_pos + t <= stop_position:
                busted = (streak_len - t) >= BUST_STEPS
                streak_depths[t].append(busted)

    for t in THRESHOLDS:
        tx_entries[t] = list(direction_changes)
        mk_entries[t] = streak_depths[t]
...
def run_experiment():
    ...
    for seed in SEEDS:
        ...
        for _ in range(N_PER_SEED):
            ...
            # v3 reversed: BR only; mk is per-T because reversed-MK = streak-depth per T

            tx_all = {t: [] for t in THRESHOLDS}
            mk_all = {t: [] for t in THRESHOLDS}

            # Per-side placeholders (Table 5 not used for Run C / reversed)
        ...
        # --- Compute results for each threshold (REVERSED POLARITY) ---
        # TX = direction-change (large pool, same across T)
        # MK = streak-depth at T (small pool)
        # Equalize by subsampling the larger pool down to the smaller per shoe.
        for t in THRESHOLDS:
            tx_full_total = sum(len(s['tx'][t]) for s in shoe_data)
            tx_full_busts = sum(sum(s['tx'][t]) for s in shoe_data)
            mk_full_total = sum(len(s['mk'][t]) for s in shoe_data)
            mk_full_busts = sum(sum(s['mk'][t]) for s in shoe_data)
            mk_full_rate = mk_full_busts / mk_full_total * 100 if mk_full_total else 0

            # Equalize per shoe: subsample whichever is larger
            eq_busts_list = []  # busts from the subsampled (large) pool
            eq_total_list = []  # equalized count per trial
            for trial in range(EQ_TRIALS):
                ...
                for s in shoe_data:
                    ...
                    ss = min(tc, mc)
                    if tc >= mc:
                        samp = random.sample(s['tx'][t], ss)
                    else:
                        samp = random.sample(s['mk'][t], ss)
                    et += ss
                    eb += sum(samp)
            ...
            #   "TX rate" displayed = subsampled direction-change rate at equalized count
            #   "MK eq rate" displayed = full streak-depth rate (small pool, used in full)
            avg_tx_eq_busts = sum(eq_busts_list) / len(eq_busts_list)
            avg_eq_total = sum(eq_total_list) / len(eq_total_list)
            tx_rate = (avg_tx_eq_busts / avg_eq_total * 100) if avg_eq_total > 0 else 0
            avg_mk_eq_busts = mk_full_busts  # MK used in full
            avg_mk_eq_rate = mk_full_rate
            tx_total = tx_full_total  # store full TX count for reporting
            tx_busts = tx_full_busts  # store full TX busts (raw direction-change busts)

            eq_std = (sum(((b / e * 100 if e > 0 else 0) - tx_rate) ** 2
            ...
            # Z-score: subsampled TX rate vs full MK rate
            p1 = tx_rate / 100
            p2 = avg_mk_eq_rate / 100
            n1 = avg_eq_total
            n2 = mk_full_total
            if n1 > 0 and n2 > 0:
                pooled = (avg_tx_eq_busts + mk_full_busts) / (n1 + n2)
                if pooled > 0:
            ...
            # Reversed advantage: TX (subsampled direction-change) / MK (streak-depth full) - 1
            # Expected sign is OPPOSITE of main (positive becomes negative or vice versa).
...
if __name__ == "__main__":
    ...
    print("v3 -- BR-only, REVERSED POLARITY (Run C)")
    print("  TX = direction-change criterion (was MK)")
    print("  MK = streak-depth criterion per T (was TX)")
    print("  Expected: advantages flip sign vs main BR-only run")
